...
# Mints valid WebAuthn ceremony payloads from a fixed EC P-256 key pair, so registration and
# authentication can be exercised end to end without a browser. Ported from fizzy's test helper.
# The relying party is "www.example.com" / origin "http://www.example.com".
module WebauthnTestHelper
  ...
  def with_webauthn_request_context
    Unmagic::Passkeys::WebAuthn::Current.host = RP_ID
    Unmagic::Passkeys::WebAuthn::Current.origin = ORIGIN
    yield
  ensure
  ...
  private
    def webauthn_client_data_json(challenge:, type:)
      { challenge: challenge, origin: ORIGIN, type: type }.to_json
    end

    def build_attestation_auth_data(credential_id:)
      [
        Digest::SHA256.digest(RP_ID),
...
RSpec.configure do |config|
  config.include WebauthnTestHelper
end
